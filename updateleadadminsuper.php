...
if (isset($_POST['submit'])) {
	...
	if(!empty($_POST['cDate'])&& !empty($_POST['cname'])&& !empty($_POST['cnumber'])&& !empty($_POST['acnumber'])&& !empty($_POST['type'])&& !empty($_POST['platform'])&& !empty($_POST['project'])&& !empty($_POST['interest'])){
...
$sql = "UPDATE leadform set id=$ids, Date='$cDate',cname='$cname',cnumber='$cnumber',acnumber='$acnumber',campaign='$campaign',type ='$type',platform='$platform'
,project='$project',interest='$interest' where id=$idsup";
	
		
		$run= mysqli_query($connectingDB, $sql)or die (mysqli_error());
		if($run){
		 header("Location: dashboardadminsuper.php?message= Lead Updated");
	                 exit();
			
		}
		else {
		 header("Location: dashboardadminsuper.php?message=Lead not Updated");
	                 exit();
		}
	}
	else {
...
<div class="form">
     <form class="form1" action="" method="post">
		 ...
		 <input type="text" name="campaign" placeholder="Campaign" value="<?php echo $arr['campaign']?>">
        ...
        <select class="select" name="project">
            <option value="<?php echo $arr['project']?>"><?php echo $arr['project']?></option>
